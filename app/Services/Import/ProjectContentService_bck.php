...
class ProjectContentService_bck
{
    ...
    private function processProject(string $projectName, string $projectPath): void
    {
        ...
        $projectId = config("myapp.projects.{$projectName}");
    ...
    private function importFile(int $projectId, string $contentTypeStr, string $filePath): void
    {
        ...
        // Determine which DTO/Model to use based on directory name
        switch ($contentTypeStr) {
            case 'categories':
                $this->handleCategoryImport($projectId, $object);
                break;
            case 'tags':
                $this->handleTagImport($projectId, $object);
                break;
            default:
